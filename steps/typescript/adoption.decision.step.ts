...
export const handler: Handlers['TsWorkflowDecision'] = async (input, { emit, logger, streams, traceId, state }) => {
  const { applicationId, petId, adopterName, adopterEmail } = input;

  logger.info('⚖️ Processing workflow decision input', { applicationId, petId, topic: input.topic || 'unknown' });
