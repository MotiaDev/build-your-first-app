...
export const handler = async (req: any, context?: any) => {
  ...
  if (emit) {
    await emit({ topic: 'ts.adoption.applied', data: { applicationId, petId, traceId } });
  }
  
  return { status: 202, body: record || { entityId: applicationId, type: 'application', phase: 'applied' } };
};
