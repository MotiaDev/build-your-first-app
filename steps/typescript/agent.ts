...
export class PetRecommendationAgent {
  
  static scorePet(pet: any, preferences: AdopterPreferences): number {
    let score = 0;
  ...
  static generateReason(pet: any, preferences: AdopterPreferences, score: number): string {
    const reasons = [];
  ...
  static async refineReasonWithAI(reason: string, petName: string): Promise<string> {
  ...
  static async getRecommendations(preferences: AdopterPreferences, limit: number = 3): Promise<PetMatch[]> {
  ...
  static async generateApplicationSummary(applicationData: any): Promise<string> {
