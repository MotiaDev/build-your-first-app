// src/typescript/treatment-scheduler.step.ts
...
export const handler: Handlers['TsTreatmentScheduler'] = async (
  input,
  { logger }
) => {
  const {
    ...
    symptoms = [],
    urgency,
  } = input as { petId: string; symptoms?: string[]; urgency?: string }
