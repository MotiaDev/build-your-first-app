// src/typescript/adoption-review-agent.step.ts
...
export const config: ApiRouteConfig = {
  ...
  method: 'POST',
  emits: ['ts.adoption.needs_data', 'ts.adoption.ready'],
  flows: ['TsPetManagement'],
}

export const handler: Handlers['TsAdoptionReviewAgent'] = async (
  req,
  { emit, logger }
) => {
  ...
  if (!['healthy', 'available'].includes(pet.status)) {
    return {
      ...
      body: {
        message:
          'Adoption review can only be performed on healthy or available pets',
        currentStatus: pet.status,
      },
    }
  }
  ...
  const recentArtifacts = getAgentArtifacts(petId).filter(
    (a) =>
  ...
  try {
    ...
    const chosenEmitDef = ADOPTION_REVIEW_EMITS.find(
      (e) => e.id === artifact.parsedDecision.chosenEmit
    )
    if (!chosenEmitDef) {
      return {
        status: 500,
