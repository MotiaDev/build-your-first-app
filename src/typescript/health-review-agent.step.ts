...
export const handler: Handlers['TsHealthReviewAgent'] = async (
  req,
  { emit, logger }
) => {
  ...
  if (!['healthy', 'in_quarantine', 'available'].includes(pet.status)) {
    return {
      ...
      body: {
        message:
          'Health review can only be performed on healthy, quarantined, or available pets',
        currentStatus: pet.status,
      },
    }
  }
  ...
  try {
    ...
    if (logger) {
      logger.info('✅ Agent decision call completed', {
        petId,
    ...
    const chosenEmitDef = HEALTH_REVIEW_EMITS.find(
