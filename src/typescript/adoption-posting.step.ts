// src/typescript/adoption-posting.step.ts
...
export const config = {
  ...
  description:
    'Posts pet for adoption and schedules adoption interviews when pet is ready',
  subscribes: ['ts.adoption.ready'],
  emits: [],
  flows: ['TsPetManagement'],
}

export const handler: Handlers['TsAdoptionPosting'] = async (
  input,
  { logger }
) => {
  ...
  try {
    ...
    const adoptionPosting = {
      ...
      postedAt: Date.now(),
      title: `${pet.name} - ${
        pet.profile?.breedGuess || pet.species
      } Available for Adoption`,
      description:
        pet.profile?.bio ||
        `Meet ${pet.name}, a lovely ${pet.species} looking for a forever home.`,
    ...
    if (logger) {
      logger.error('❌ Adoption posting failed', {
        petId,
        error: error.message,
      })
    }
