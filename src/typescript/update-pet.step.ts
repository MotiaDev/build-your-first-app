...
export const handler: Handlers['TsUpdatePet'] = async (
  req,
  { emit, logger }
) => {
  try {
    ...
    if (validatedData.species !== undefined)
      patch.species = validatedData.species
    if (validatedData.ageMonths !== undefined)
      patch.ageMonths = validatedData.ageMonths
    if (validatedData.notes !== undefined) patch.notes = validatedData.notes
    if (validatedData.nextFeedingAt !== undefined)
      patch.nextFeedingAt = validatedData.nextFeedingAt

    const updated = TSStore.update(id, patch)
    return updated
      ? { status: 200, body: updated }
      : { status: 404, body: { message: 'Pet not found' } }
  } catch (error) {
