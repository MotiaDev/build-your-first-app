    ...
    switch (guard) {
      case 'must_be_healthy':
        if (pet.status !== 'healthy') {
          return {
            passed: false,
            reason: `Pet must be healthy (current: ${pet.status})`,
          }
        }
        break
      case 'no_needs_data_flag':
        if (pet.flags?.includes('needs_data')) {
          return {
            passed: false,
            reason: 'Pet has needs_data flag blocking adoption',
          }
        }
        break
      default:
...
export const handler: Handlers['TsPetLifecycleOrchestrator'] = async (
  input,
  { emit, logger, streams, traceId }
) => {
  ...
  try {
    ...
    if (emit && updatedPet) {
      ...
      await processAutomaticProgression(
        petId,
        rule.to,
        emit,
        logger,
...
async function emitNextActionEvents(
  petId: string,
  ...
  try {
    ...
    if (logger) {
      logger.error('❌ Failed to emit next action events', {
        petId,
        newStatus,
        error: error.message,
      })
...
async function processAutomaticProgression(
  ...
  const automaticProgressions: Partial<
    Record<Pet['status'], { to: Pet['status']; description: string }>
  > = {
    healthy: {
      to: 'available',
